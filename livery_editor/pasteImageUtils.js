...
function addPastCanvasEventListeners(pasteCanvas) {
  ...
  pasteCanvas.addEventListener("mousemove", (evt) => {
    if (isMovingPasteCanvas) {
      ...
      if (currY < lastOffsetHeight) {
        pasteCanvas.style.top =
          parseInt(pasteCanvas.style.top) - offsetY + "px";
      } else {
        pasteCanvas.style.top =
          parseInt(pasteCanvas.style.top) + offsetY + "px";
      }
      ...
      if (currX < lastOffsetWidth) {
        pasteCanvas.style.left =
          parseInt(pasteCanvas.style.left) - offsetX + "px";
      } else {
        pasteCanvas.style.left =
          parseInt(pasteCanvas.style.left) + offsetX + "px";
      }
  ...
  pasteCanvas.addEventListener("mouseup", (evt) => {
    ...
    const pasteData = pasteCanvas
      .getContext("2d")
      .getImageData(0, 0, pasteCanvas.width, pasteCanvas.height).data;
    const pasteImgSectionData = [];
    for (
      let row = pasteImgRowStart * 4 * pasteCanvas.width;
      row < pasteImgRowEnd * 4 * pasteCanvas.width;
      row += 4 * pasteCanvas.width
    ) {
